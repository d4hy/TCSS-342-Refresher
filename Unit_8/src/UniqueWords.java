...
public class UniqueWords {
    ...
    public  MyBinarySearchTree avlOfUniqueWords;
    ...
    // Adds the unique words to a self-balancing
    //MyBinarySearchTree.
    // For each word in the list stored in book:
    ...
    public void addUniqueWordsToAVL(){
        ...
        String currentWord = book.words.first();
        while(currentWord!=null) {
            if (avlOfUniqueWords.find(currentWord) != currentWord) {
                avlOfUniqueWords.add(currentWord);
            }
            currentWord = book.words.next();
        ...
        Instant addEndTime = Instant.now();
        Instant traverseStartTime = Instant.now();
        avlOfUniqueWords.toString();

        Instant traverseEndTime = Instant.now();
        System.out.println("Adding unique words to an AVL binary search tree... in  " +  Duration.between(addStartTime , addEndTime).toMillis() + "ms.\n");
        System.out.println(avlOfUniqueWords.size()+" unique words");
        System.out.println( avlOfUniqueWords.height()+" height");
        System.out.println(avlOfUniqueWords.comparisons+ " comparisons");
        System.out.println(avlOfUniqueWords.rotations +"rotations ");
        System.out.println("Traversing the AVL... in  "+ Duration.between(traverseStartTime , traverseEndTime).toMillis() + "ms.\n");
    }
    ...
    public void addUniqueWordsToHashTable() {
        ...
        String currentWord = book.words.first();
        while (currentWord != null) {
            if (!Objects.equals(hashOfUniqueWords.get(currentWord), currentWord)) {
                hashOfUniqueWords.put(currentWord, currentWord);
            }
            currentWord = book.words.next();
        ...
        System.out.println("Adding unique words to a hash table... in " + getTimeDiff(startTime, endTime) + " seconds");
        System.out.println(hashOfUniqueWords.size() + " unique words.");
        System.out.println(hashOfUniqueWords.comparisons + " comparisons.");
        System.out.println(hashOfUniqueWords.maxProbe + " max probe.");

        startTime = System.currentTimeMillis();
        hashOfUniqueWords.toString();
        endTime = System.currentTimeMillis();
        System.out.println("Extracting the key-value pairs... in " + getTimeDiff(startTime, endTime) + " seconds.");
    ...
    /*
     *  Helper method to calculate time difference a bit easier.
     * */
    private double getTimeDiff(final long startTime, final long endTime) {
